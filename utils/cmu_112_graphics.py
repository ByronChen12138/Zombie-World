...
class WrappedCanvas(Canvas):
    ...
    def log(self, methodName, args, kwargs):
        if (not self.inRedrawAll):
            self.app._mvcViolation('you may not use the canvas (the views) outside of redrawAll')
...
class App(object):
    ...
    # User Methods:
    ####################################
    def redrawAll(app, canvas): pass      # draw (views) the model in the canvas
    ...
    def __setattr__(app, attr, val):
        ...
        if (d.get('running', False) and
            ...
            canvas.inRedrawAll):
            app._mvcViolation(f'you may not change app.{attr} in the model while in redrawAll (the views)')
    ...
    def _redrawAllWrapper(app):
        ...
        try:
            ...
            if (hash1 != hash2):
                app._mvcViolation('you may not change the app state (the model) in redrawAll (the views)')
        finally:
